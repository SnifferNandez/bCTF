$('document').ready(function() {
    update_feed();
    setInterval(update_feed, 15000); // Update scores every 15 seconds
})
...
function feed_table(element) {
    var show_location = ``;
    if (element.attendant) 
        show_location += `<img src="/static/bsidesco/static/img/guecha.png" height="40">`;
    if (element.country) 
        show_location += `<i class="flag2x ${element.country}"></i>`;
    var show_rank = ``;
    if (element.rank == 1 )
        show_rank += `<i class="fas fa-crown" style="color:goldenrod"></i>`;
    if (element.rank == 2 )
        show_rank += `<i class="fas fa-medal" style="color:silver"></i>`;
    if (element.rank == 3 )
        show_rank += `<i class="fas fa-medal" style="color:#CD7F32"></i>`;
    $('#scores-body').append(
        `
        <tr>
            <td class="td-points">${element.rank} ${show_rank}</td>
            <td>
                ...
                <div id="team_info" style="margin-left: 70px;">
                    ...
                    <p>Progreso ${element.precentage}%</p>
                </div>
            </td>
            <td>${show_location}</td>
...
function scores_graph() {
    $.get('/api/top/', function( data ) {
        ...
        if (Object.keys(ranks).length === 0 ){
            ...
            $('#scoreboard').html(
                '<div class="center-align"><h3>Sin soluciones aun</h3></div>'
            );
            return;
        }
